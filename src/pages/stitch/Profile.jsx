...
import { User, Trash2, AlertTriangle, Edit2, Check, X, Clock, Bell } from 'lucide-react';
        ...
        <div className="min-h-screen bg-gray-50 p-4 pb-24">
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                            Teléfono 📱 {userData?.telefono_verificado && <span className="text-green-600 ml-2">✓</span>}
                        </label>
            ...
            <div className="bg-white rounded-xl shadow-md overflow-hidden">
                <button onClick={() => navigate('/history')} className="w-full flex items-center gap-3 p-4 hover:bg-gray-50 border-b border-gray-100">
                    <Clock className="w-5 h-5 text-gray-500" /><span className="text-gray-800">Historial</span>
                </button>
                
                {isAdmin && (
                    <>
                        <button onClick={() => navigate('/users')} className="w-full flex items-center gap-3 p-4 hover:bg-gray-50 border-b border-gray-100">
                            <User className="w-5 h-5 text-purple-600" /><span className="text-gray-800">Gestionar Usuarios</span>
                        </button>
                        <button onClick={() => navigate('/admin/notifications')} className="w-full flex items-center gap-3 p-4 hover:bg-gray-50 border-b border-gray-100">
                            <Bell className="w-5 h-5 text-purple-600" /><span className="text-gray-800">Notificaciones</span>
                        </button>
                    </>
                )}
                
                <button onClick={handleLogout} className="w-full flex items-center gap-3 p-4 hover:bg-gray-50 text-red-600">
                    <X className="w-5 h-5" /><span>Cerrar Sesión</span>
                </button>
            </div>
